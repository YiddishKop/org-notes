...
** agent-pc sufer net by sharing the server-laptop's wirless netcard

   one pc without wireless net-card, one laptop with two net-cards: wired and wireless.
   so i want to make pc surfer net by sharing laptop's wireless net-card, using the NAT
   tech.
   step-by-step, as follow note:
*** Set

    sudo iptables -A FORWARD -o wlp8s0 -i enp7s0 -s 10.42.0.1/24 -m conntrack --ctstate NEW -j ACCEPT
